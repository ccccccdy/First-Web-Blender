<template>
  <div v-if="visible && asset" class="modal-overlay" @click.self="close">
    <div class="modal-container">
      <div class="title-area">
        <input v-if="isEditing" v-model="editForm.title" class="edit-title-input" placeholder="资产名称" />
      ...
      <div class="modal-body">
        ...
        <div class="description-area">
          <textarea v-if="isEditing" v-model="editForm.description" rows="4" class="edit-description-input" placeholder="资产描述"></textarea>
          <p v-else class="description">{{ asset.description || '暂无描述' }}</p>
        </div>

        <div v-if="isSyncFailedStatus && asset.failureReason" class="failure-box">
          <strong>未通过原因</strong>
        ...
        <div class="info">
          ...
          <span>{{ RESOURCE_SIZE_LABEL }}：{{ formatSize(asset.fileSize) }}</span>
        </div>
        ...
        <div v-if="!isEditing || isPendingLinkStatus || isSyncingStatus || isSyncedStatus" class="panel-block">
            ...
            <div class="quark-grid">
              <div class="quark-row">
                ...
                <template v-if="asset.quarkUrl">
                  ...
                  <span v-else class="quark-value">{{ asset.quarkUrl }}</span>
                </template>
                <span v-else class="quark-empty">暂未填写</span>
              </div>
              <div class="quark-row">
                <span class="quark-label">提取码</span>
                <span class="quark-value">{{ asset.extractionCode || '暂未填写' }}</span>
              </div>
            </div>
          ...
          <p v-if="isSyncingStatus" class="waiting-note">等待管理页审核，当前资源在普通页面中只读。</p>
        </div>
        ...
        <div v-if="isEditing" class="panel-block">
          ...
          <div class="edit-tip">
            当前位置只允许修改标题、描述、夸克链接和提取码；本地文件暂不支持二次替换。
          </div>
        </div>
      </div>
      ...
      <div class="modal-buttons">
        <template v-if="!isEditing">
          ...
          <button class="delete-btn" @click="confirmDelete">删除资源</button>
        </template>
        <template v-else>
...
const syncStatus = computed(() => props.asset?.syncStatus)
const statusMeta = computed(() => getStatusMeta(syncStatus.value))
const fileNames = computed(() => splitUrls(props.asset?.fileUrls).map(extractFileName))
...
const enterEditMode = () => {
  ...
  editForm.title = props.asset.title
  editForm.description = props.asset.description || ''
  editForm.quarkUrl = props.asset.quarkUrl || ''
  editForm.extractionCode = props.asset.extractionCode || ''
}
...
const buildPayload = (action) => {
  ...
  formData.append('id', props.asset.id)
...
const saveDraft = async () => {
  try {
    await axios.put('/api/assets', buildPayload('save'), {
...
const submitReviewAction = async () => {
  ...
  try {
    await axios.put('/api/assets', buildPayload(action), {
      headers: { 'Content-Type': 'multipart/form-data' }
    })
...
const confirmDelete = () => {
  if (confirm(`确定要删除资产“${props.asset.title}”吗？此操作不可恢复。`)) {
