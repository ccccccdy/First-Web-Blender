<template>
  <div v-if="visible && artwork" class="modal-overlay" @click.self="close">
    <div class="modal-container">
      <div class="title-area">
        <input v-if="isEditing" v-model="editForm.title" class="edit-title-input" placeholder="作品名称" />
        <h2 v-else>{{ artwork.title }}</h2>
      </div>
      ...
      <div class="modal-body">
        ...
        <div v-if="previewImageUrl" class="cover">
          <img :src="previewImageUrl" :alt="artwork.title" class="cover-media" />
        </div>

        <div class="description-area">
          <textarea v-if="isEditing" v-model="editForm.description" rows="4" class="edit-description-input" placeholder="作品描述"></textarea>
          <p v-else class="description">{{ artwork.description || '暂无描述' }}</p>
        </div>

        <div v-if="isSyncFailedStatus && artwork.failureReason" class="failure-box">
          <strong>未通过原因</strong>
          <p>{{ artwork.failureReason }}</p>
        </div>

        <div class="info">
          <span>创建时间：{{ formatDate(artwork.createdAt) }}</span>
          <span>{{ RESOURCE_SIZE_LABEL }}：{{ formatSize(artwork.fileSize) }}</span>
        </div>
        ...
        <div v-if="!isEditing || isPendingLinkStatus || isSyncingStatus || isSyncedStatus" class="panel-block">
          ...
          <div class="quark-content-row">
            <div class="quark-grid">
              <div class="quark-row">
                ...
                <template v-if="artwork.quarkUrl">
                  <a
                    v-if="isSyncedStatus || isSyncingStatus"
                    :href="artwork.quarkUrl"
                    target="_blank"
                    rel="noreferrer"
                    class="quark-link"
                  >
                  ...
                  <span v-else class="quark-value">{{ artwork.quarkUrl }}</span>
                </template>
                <span v-else class="quark-empty">暂未填写</span>
              </div>
              <div class="quark-row">
                <span class="quark-label">提取码</span>
                <span class="quark-value">{{ artwork.extractionCode || '暂未填写' }}</span>
              </div>
            </div>
          ...
          <p v-if="isSyncingStatus" class="waiting-note">等待管理页审核，当前作品在普通页面中只读。</p>
        </div>

        <div v-if="!isSyncedStatus" class="panel-block">
          <strong class="section-title">{{ ARTWORK_LOCAL_INFO_TITLE }}</strong>
          <div v-if="previewImageUrl" class="preview-inline">
            <a :href="previewImageUrl" target="_blank" download class="preview-thumb-link">
              <img :src="previewImageUrl" :alt="artwork.title" class="preview-thumb" />
            </a>
          </div>
          <ul v-if="modelFileNames.length" class="plain-list">
            <li v-for="name in modelFileNames" :key="name">{{ name }}</li>
          </ul>
          <p v-else class="empty-text">本地压缩包已不可用</p>
        </div>

        <div v-if="isSyncedStatus" class="panel-block">
          <strong class="section-title">交付信息</strong>
          <p class="delivery-note">审核通过后，本地压缩包已删除，请使用夸克链接和提取码取用资源。预览图仍可保留和下载。</p>
          <a v-if="previewImageUrl" :href="previewImageUrl" target="_blank" download class="preview-download">下载预览图</a>
        </div>
        ...
        <div v-if="isEditing" class="panel-block">
          ...
          <div class="edit-tip">
            当前阶段只允许修改标题、描述、夸克链接和提取码；上传后的预览图和本地压缩包不再替换。
          </div>
        </div>
      </div>
      ...
      <div class="modal-buttons">
        <template v-if="!isEditing">
          ...
          <button class="delete-btn" @click="confirmDelete">删除作品</button>
        </template>
        <template v-else>
...
import { ARTWORK_LOCAL_INFO_TITLE } from '../utils/artworkText'
...
const syncStatus = computed(() => props.artwork?.syncStatus)
const statusMeta = computed(() => getStatusMeta(syncStatus.value))
const coverFileUrls = computed(() => splitUrls(props.artwork?.coverUrls))
const modelFileNames = computed(() => splitUrls(props.artwork?.modelFileUrls).map(extractFileName))
const previewImageUrl = computed(() => coverFileUrls.value[0] || '')
...
const enterEditMode = () => {
  ...
  editForm.title = props.artwork.title
  editForm.description = props.artwork.description || ''
  editForm.quarkUrl = props.artwork.quarkUrl || ''
  editForm.extractionCode = props.artwork.extractionCode || ''
}
...
const buildPayload = (action) => {
  ...
  formData.append('id', props.artwork.id)
...
const saveDraft = async () => {
  try {
    await axios.put('/api/artworks', buildPayload('save'), {
...
const submitReviewAction = async () => {
  ...
  try {
    await axios.put('/api/artworks', buildPayload(action), {
      headers: { 'Content-Type': 'multipart/form-data' }
    })
...
const confirmDelete = () => {
  if (confirm(`确定要删除作品“${props.artwork.title}”吗？此操作不可恢复。`)) {
    emit('delete', props.artwork.id)
  }
}
</script>
...
.modal-container {
  background: linear-gradient(135deg, rgba(255, 255, 255, 0.6), rgba(0, 0, 0, 0.72));
